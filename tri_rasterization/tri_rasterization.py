...
import cv2
def render_colors_tri_me(img_c,mask,vertices_c,vertices_t, triangles, h, w, c = 3):
    ''' render mesh with colors
    Args:
        vertices: [nver, 3]
        triangles: [ntri, 3] 
    ...
    Returns:
        image: [h, w, c]. 
    '''
    assert vertices_c.shape[0] == vertices_t.shape[0]
    
    H,W = h,w 
    c = np.empty((H, W, 2))
    c[..., 0] = np.linspace(0, W, W, dtype=np.float32)
    c[..., 1] = np.expand_dims(np.linspace(0, H, H, dtype=np.float32), -1)
    grid=c
    grid[np.where(mask==[255,255,255])[:2]]=np.array([0,0])
    ...
    for i in range(triangles.shape[0]):
        ...
        # the inner bounding box
        umin = max(int(np.ceil(np.min(vertices_t[tri, 0]))), 0)
        umax = min(int(np.floor(np.max(vertices_t[tri, 0]))), w-1)

        vmin = max(int(np.ceil(np.min(vertices_t[tri, 1]))), 0)
        vmax = min(int(np.floor(np.max(vertices_t[tri, 1]))), h-1)
        ...
        for u in range(umin, umax+1):
            for v in range(vmin, vmax+1):
                if not isPointInTri([u,v], vertices_t[tri, :2]): 
                    continue
                w0, w1, w2 = get_point_weight([u, v], vertices_t[tri, :2])
                point_depth = w0*vertices_t[tri[0], 2] + w1*vertices_t[tri[1], 2] + w2*vertices_t[tri[2], 2]

                if point_depth > depth_buffer[v, u]:
                    depth_buffer[v, u] = point_depth
                    grid[v,u,:]=w0*vertices_c[tri[0], :2] + w1*vertices_c[tri[1], :2] + w2*vertices_c[tri[2], :2]
                    
                    
        mx = (grid[:, :, 0]).astype(np.float32)
        my = (grid[:, :, 1]).astype(np.float32)                    
    return cv2.remap(img_c, mx, my, cv2.INTER_CUBIC)
