...
def tps_grid(theta, c_dst, dshape):    
    ...
    print('tps.numpy uniform_grid time: ',time.time()-st_t,'s')
    
    reduced = c_dst.shape[0] + 2 == theta.shape[0]
    
    dx = TPS.z(ugrid.reshape((-1, 2)), c_dst, theta[:, 0]).reshape(dshape[:2])
    print('tps.numpy z x time: ',time.time()-st_t,'s')
    dy = TPS.z(ugrid.reshape((-1, 2)), c_dst, theta[:, 1]).reshape(dshape[:2])
    print('tps.numpy z y time: ',time.time()-st_t,'s')
    dgrid = np.stack((dx, dy), -1)
    print('tps.numpy stack time: ',time.time()-st_t,'s')
    grid = dgrid + ugrid
    
    return grid # H'xW'x2 grid[i,j] in range [0..1]

def tps_grid_mask(theta, c_dst, dshape,mask):    
    st_t=time.time()
    ugrid = uniform_grid(dshape)
    face_idx=(np.array(np.where(mask==[255,255,255]))[:2]).T
    ...
    dx = TPS.z(face_idx, c_dst, theta[:, 0])
    print('tps.numpy z x time: ',time.time()-st_t,'s')
    dy = TPS.z(face_idx, c_dst, theta[:, 1])
    print('tps.numpy z y time: ',time.time()-st_t,'s')
    dgrid_face = np.stack((dx, dy), -1)
    print('tps.numpy stack time: ',time.time()-st_t,'s')
    grid = ugrid
    print(np.where(mask==[255,255,255])[:2])
    grid[np.where(mask==[255,255,255])[:2]]+=dgrid_face
